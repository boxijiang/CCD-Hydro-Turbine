...
outputDir = "oldcode_test_FDtotale6"
...
phase = traj.add_phase(
    "phase0",
    dm.Phase(
        ode_class=TurbineODE,
        transcription=dm.GaussLobatto(num_segments=num_seg, compressed=True),
        ode_init_kwargs={"partials_method": "FD"},
    ),
)
phase.add_state(
    "x", fix_initial=False, lower=0, rate_source="xdot", units=None, targets="x"
)  # target required because x0 is an input
phase.add_state("J", fix_initial=True, fix_final=False, lower=0.0, rate_source="Jdot", units=None, ref=1e6)

# phase.add_control(name='u', units=None, continuity=True, rate_continuity=True, targets='u')
phase.add_control(
    name="u", units=None, lower=0, upper=80000, continuity=True, rate_continuity=True, targets="u", ref=1e5
)
...
phase.add_parameter("mass", targets=["ode_comp.mass"], static_target=True, units=None, val=2234, opt=False)
t_final = 20
phase.set_time_options(
    fix_initial=True, fix_duration=True, duration_val=t_final, units="s", targets="torque_comp.time_phase"
)
...
prob.setup()
prob.model.approx_totals(method="fd", step=1e-6, form="central", step_calc="rel")
